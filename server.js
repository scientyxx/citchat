...
socket.on('chatMessage', async (data) => {
    ...
    if (message === '!time') {
      // Jika pesan khusus adalah "!time", kirim waktu server hanya ke klien ini
      const serverTimeWIB = new Date().toLocaleTimeString('en-US', { timeZone: 'Asia/Jakarta' });
      io.to(socket.id).emit('serverTime', { time: serverTimeWIB });
    } else {
...
//     const serverTimeWIB = new Date().toLocaleTimeString('en-US', { timeZone: 'Asia/Jakarta' });
//     io.to(socket.id).emit('serverTime', { time: `Server Time (WIB): ${serverTimeWIB}` });
//   });
